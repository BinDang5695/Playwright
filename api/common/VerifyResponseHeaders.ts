...
export class VerifyResponseHeaders {
  ...
  static verify(response: APIResponse) {
    ...
    expect(headers['keep-alive'])
      .toContain('timeout=5, max=100');

    expect(headers['cache-control'])
      .toContain('no-cache, private');
    ...
    expect(headers['content-encoding'])
      .toContain('gzip');
    ...
    expect(
      new Date(dateHeader!).toString()
    ).not.toBe('Invalid Date');

    expect(headers['server'])
      .toContain('LiteSpeed');
  }
}